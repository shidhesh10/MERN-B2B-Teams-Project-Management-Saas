...
export const getProjectAndAnalyticsController = asyncHandler(
    async(req: Request, res: Response) => {
        const projectId = projectIdSchema.parse(req.params.id);
        const workspaceId = workspaceIdSchema.parse(req.params.workspaceId);
        ...
        const {analytics} = await getProjectAndAnalyticsService(
            workspaceId,
            projectId,
        );

        return res.status(HTTPSTATUS.OK).json({
            message: "Project analytics retrived successfully",
            analytics,
        });
    }
);
